...
module.exports = {
  async up(queryInterface, Sequelize) {
    console.log('🚀 Updating to NovaSearch/stella_en_400M_v5 with 1024-dimensional embeddings...');
    ...
    for (const tableName of tables) {
      console.log(`🔄 Updating ${tableName} table to 1024-dimensional stella embeddings...`);
  ...
  async down(queryInterface, Sequelize) {
    console.log('🔄 Reverting to 384-dimensional embeddings...');
    
    const tables = ['alerts', 'incidents', 'assets', 'iocs', 'playbooks'];
    
    for (const tableName of tables) {
      console.log(`🔄 Reverting ${tableName} table to 384 dimensions...`);
      ...
      // Clear all existing embedding data
      await queryInterface.sequelize.query(`
        UPDATE "${tableName}" SET embedding = NULL;
      `);
