...
const EditLineModal: React.FC<EditLineModalProps> = ({
  ...
  // Initialize form data when line changes
  useEffect(() => {
    if (line) {
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!line || !validateForm()) {
      return;
    }

    setIsSubmitting(true);
    try {
      await logAnalyzerService.updateLine(fileId, line.id, {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to update line';
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block w-full max-w-lg p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-soc-dark-900 shadow-xl rounded-lg border border-gray-200 dark:border-soc-dark-700">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Line Info */}
            <div className="bg-gray-50 dark:bg-soc-dark-800 p-3 rounded-lg">
              <div className="text-sm text-gray-600 dark:text-gray-400">
                <strong>Line Number:</strong> {line.lineNumber}
              </div>
              {line.parsedData && Object.keys(line.parsedData).length > 0 && (
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  <strong>Parsed Data:</strong> {JSON.stringify(line.parsedData, null, 2)}
                </div>
              )}
            </div>
